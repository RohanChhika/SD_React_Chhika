...
const AdminBlockUsersButton = () => {
  ...
  return (
    <>
      {/* Render the View Users button only if userInfo is available and user role is 'admin' */}
      {userInfo && userRoleRef.current === 'admin' && (
        <Link to="/admin-handle-users">   {/* Link to the admin-handle-users route */}
          <button className='button'>View Users</button>  {/* Button to view users */}
        </Link>
      )}
    </>
  );
...
export default AdminBlockUsersButton;
