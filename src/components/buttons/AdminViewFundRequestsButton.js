...
const AdminViewFundRequestsButton = () => {
  ...
  return (
    <>
    {/* Render the View Fund Manager Requests button only if userInfo is available and user role is 'admin' */}
      {userInfo && userRoleRef.current === 'admin' && (
        <Link to="/admin-fund-manager-requests">    {/* Link to the admin-fund-manager-requests route */}
          <button className='button'>View Fund Manager Requests</button>    {/* Button to view fund manager requests */}
        </Link>
      )}
    </>
  );
