...
// describe('ViewMyFundRequestsButton Component', () => {
//   beforeEach(() => {
//     // Mock successful fetch with a manager role
//     global.fetch = jest.fn(() =>
//       Promise.resolve({
//         ok: true,
//         json: () => Promise.resolve({ role: 'manager' }) // Simulating a manager user
//       })
//     );
//   });
...
//   test('renders button for managers', async () => {
...
//     await screen.findByText('View My Fund Requests'); // Await the button to appear
//     expect(screen.getByText('View My Fund Requests')).toBeInTheDocument();
//     expect(screen.getByRole('link')).toHaveAttribute('href', '/view-my-fund-requests');
//   });

//   test('does not render button for non-manager users', async () => {
//     useAuth0.mockReturnValue({
//       user: { sub: '123' },
//       isAuthenticated: true,
//       getAccessTokenSilently: jest.fn(() => Promise.resolve('fake_token')),
...
//       Promise.resolve({
//         ok: true,
//         json: () => Promise.resolve({ role: 'user' }) // Simulating a non-manager user
//       })
//     );
...
//     const button = screen.queryByText('View My Fund Requests');
